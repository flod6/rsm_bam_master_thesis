...
# Note: Old GNN Objects before the DGL implementation
# Just to save them for the case they don't work
...
def PyTorchGraphLoader(dataset):
    ...
    # Define splits
    train_idx = list(range(0, 54)) # 2000Q1 – 2013Q4
    val_idx = list(range(52, 68)) # 2014Q1 – 2017Q4
    test_idx = list(range(68, 94)) # 2018Q1 – 2023Q4
...
class GraphSage(nn.Module):
     ...
     def forward(self, x, edge_index, batch=None):
         # Sort edge_index for lstm  
         edge_index = torch_geometric.utils.sort_edge_index(
                         edge_index, 
            num_nodes=x.size(0), 
            sort_by_row=False
         )
         # print("→ [DEBUG] raw edge_index.shape =", edge_index.shape)
...
def make_table(val_loader, test_loader, model, device):
    ...
    with torch.no_grad():
        for graph in val_loader:
            ...
            all_targets_val.append(graph.y.cpu().numpy().astype(np.float64))
    ...
    with torch.no_grad():
        for graph in test_loader:
            ...
            all_targets_test.append(graph.y.cpu().numpy())
    ...
    return results_df, all_preds_val, all_targets_val 



# Function to get predictions for every node and graph for all splits
