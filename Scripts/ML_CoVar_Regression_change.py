...
# Isolate variables
y_train = train["diff_delta_covar"]
x_train = train.drop(columns= ["date", "gvkey", "diff_delta_covar" , "lag_covar", "covar"
                               ])
y_val = val["diff_delta_covar"]
x_val = val.drop(columns= ["date", "gvkey", "diff_delta_covar" , "lag_covar", "covar"
                           ])
y_test = test["diff_delta_covar"]
x_test = test.drop(columns= ["date", "gvkey", "diff_delta_covar" , "lag_covar", "covar"
                             ])
...
# Lasso
lasso_tuning_grid = {"model__alpha": np.logspace(-3, 1, 100)}
...
#print(f"RF: Best NE: {rf_wf.best_params_['n_estimators']}")
# print(f"RF: Best MD: {rf_wf.best_params_['max_depth']}")
# print(f"RF: Best MinSamp: {rf_wf.best_params_['min_samples_split']}")
# print(f"RF: Best MinLeaf: {rf_wf.best_params_['min_samples_leaf']}")
# print(f"RF: Best Boot: {rf_wf.best_params_['bootstrap']}")
...
summary_df.to_csv(output + "Result_Models/ML_CoVar_Regression_change_results.csv", index=False)

# Save the models locally
joblib.dump(best_linreg_model, output + "Models/linreg_CoVar_change_absolute.pkl")
joblib.dump(best_lasso_model, output + "Models/lasso_CoVar_change_absolute.pkl")
joblib.dump(best_rf_model, output + "Models/rf_CoVar_change_absolute.pkl")
...
feature_importances.to_csv(output + "Feature_Importance/features_CoVar_regression_change.csv", index=False)
...
predictions.to_csv(output + "Predictions_Models/ML_CoVar_Regression_change_predictions.csv", index=False)
